...
module.exports = async function (plugin, addressSpace) {
  ...
  plugin.onChange('extra', async recs => {
    plugin.log('onExtra ' + util.inspect(recs, null, 4), 2);
    try {
      ...
      for (const op in groupByOp) {
        ...
        if (op == 'delete') {
          ...
          deleteOpcNodesForFilters(groupByOp[op].ref);
          groupByOp[op].ref.forEach(item => {
            Object.assign(item, item.$set)
          })
  ...
  function deleteDeviceNode(key, did) {
    ...
    if (devNode) namespace.deleteNode(devNode.nodeId);
      ...
      for (const property in item.props) {
        try {
          if (
            ...
            filter.devices[item._id + '.' + property].value = item.props[property].value;
            filter.devices[item._id + '.' + property].chstatus = item.props[property].chstatus;
            filter.devices[item._id + '.' + property].ts = item.props[property].ts;
            ...
            try {
              variable = namespace.addVariable({
                ...
                minimumSamplingInterval: item.minSamplingInterval || 1000,
                rolePermissions,
                value: {
                  timestamped_get() {
  ...
  function deleteOpcNodesForFilters(arr) {
    ...
    arr.forEach(fobj => {
      try {
        ...
        if (key == "Devices") {
          deleteDeviceNode(key, fobj.did);
        } else {
          for (const did in filter.folders[key].devNodes) {
            deleteDeviceNode(key, did);
          }
          ...
          if (filter.folders[key].subfolderNodes) {
            for (const subid in filter.folders[key].subfolderNodes) {
              ...
              if (subNode) {
                namespace.deleteNode(subNode);

              }
            }
          }
          // Удалить основную папку
          if (filter.folders[key].folderNode) {
            namespace.deleteNode(filter.folders[key].folderNode);
          }
